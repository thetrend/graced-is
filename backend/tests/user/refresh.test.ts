...
describe('user-refresh handler', () => {
  test('should return 200 with a new access token', async () => {
    ...
    expect(response.body).toEqual(JSON.stringify({ accessToken: 'newAccessToken' }));
  });
  ...
  test('should return 400 if refresh token is missing', async () => {
    ...
    expect(response.body).toEqual(JSON.stringify({ message: 'Refresh token is required' }));
  });
  ...
  test('should return 401 if refresh token is invalid', async () => {
    ...
    expect(response.body).toEqual(JSON.stringify({ message: 'Invalid refresh token' }));
  });
  ...
  test('should return 500 if a database error occurs', async () => {
    ...
    expect(response.body).toEqual(JSON.stringify({ message: 'An internal error occurred' }));
  });
});
